...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		mess = {
			...
			success: 'Done Jangan Lupa Bilang makasih sama reza nya',
			error: {
...
if (budy.includes(`Hi Refy`)) {

reply(`Hallo Kak *${pushname}😁*\nAda Yang Bisa Refy Bantu?\nKetik #menu Kak Untuk Mulai Menggunakan RefyBotz`)
}
...
//┅┅┅┅〔 KALO ADA BUG LAPORKAN!! 〕┅┅┅┅/
			case 'blackpink2': 
                    if (args.length < 1) return reply(`Teks Nya Mana Tod *${pushname}?*😒\nContoh ${prefix}${command} Refy Bot`)
					mak = body.slice(11)
					reply(mess.wait)
			...
			case 'naruto': 
					...
					break 
	       case 'nulis':
           if (args.length == 0) return reply(`Teks nya mana Tod ${pushname}\nExample: ${prefix + command} RefyBotz`)
           ini_txt = args.join(" ")
...
//»»»»»»»»»»»»»»»»»»»»»»|| INI NSRW BANG ||«««««««««««««««««««««««//
           ...
           case 'nsfw':
	        ...
	        if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('Sudah Aktif Tod')
						nsfww.push(from)
						fs.writeFileSync('./database/nsfww.json', JSON.stringify(nsfww))
						reply('Sukses mengaktifkan fitur nsfw')
						fdhl.sendMessage(from, `Bebas Nyari Hentai 🗿`, text)
					} else if (Number(args[0]) === 0) {
						if (!isNsfw) return reply('Sudah Mati Tod')
...
// MAU NYOLONG? BAGI WM GW TOD!! GAK BAGI SIAP DI BULY
     ...
     case 'Refymodex' :
         ...
         but = [
         { buttonId: '#Refymodexon', buttonText: { displayText: 'ON' }, type: 1 },
         { buttonId: '#Refymodexoff', buttonText: { displayText: 'OFF' }, type: 1 }
         ]
         sendButton(from, "*SILAHKAN PILIH UNTUK REFY MODE X*", faketeks, but, {quoted: mek})
         break
     case 'Refymodexon' :
         if (!isGroup) return reply(mess.only.group)
         if (!isOwner) return reply('*FITUR HANYA TERSEDIA UNTUK OWNER*')
         if (isCikmod) return reply('*REFY MODE X AKTIF!!*')
         cikomode.push(from)
         fs.writeFileSync('./database/cikomode.json', JSON.stringify(cikomode))
         reply(`\`\`\`SUKSES MENGAKTIFKAN REFY MODE X DI GROUP\`\`\` *${groupMetadata.subject}*`)
         break
     case 'Refymodexoff' :
         if (!isGroup) return reply(mess.only.group)
         if (!isOwner) return reply('*FITUR HANYA TERSEDIA UNTUK OWNER*')
         if (!isCikmod) return reply('*REFY MODE X SUDAH AKTIF SEBELUMNYA!*')
         cikomode.splice(from, 1)
         fs.writeFileSync('./database/cikomode.json', JSON.stringify(cikomode))
         reply(`\`\`\`SUKSES MENONAKTIFKAN REFY MODE X DI GROUP\`\`\` *${groupMetadata.subject}*`)
         break
...
//»»»»»»»»»»»»»»»»»»»»»»|| menu + fake reply ||«««««««««««««««««««««««//
                   ...
                   case 'menu':
                   case 'help':
                   inimenu = `Hy Tod ${pushname} ${ucapanWaktu}
   ≡ *${botname} menu*    
  🎟${prefix}groupmenu
  🎟${prefix}stickermenu
  🎟${prefix}funmenu
...
//»»»»»»»»»»»»»»»»»»»»»»|| group menu ||«««««««««««««««««««««««//
                    ...
                    case 'groupmenu':                            
                    gcmenu = `${ucapanWaktu} Tod ${pushname}
...
${wmaing}`
 ...
 //»»»»»»»»»»»»»»»»»»»»»»|| sticker menu ||«««««««««««««««««««««««//                                  
                                   ...
                                   case 'stickermenu':                            
                    smenu = `${ucapanWaktu} Tod ${pushname}
...
//»»»»»»»»»»»»»»»»»»»»»»|| fun menu ||«««««««««««««««««««««««//	               
	               ...
	               case 'funmenu':                            
                    fmenu = `${ucapanWaktu} Tod ${pushname}
...
//»»»»»»»»»»»»»»»»»»»»»»|| random menu ||«««««««««««««««««««««««//
                     ...
                     case 'randommenu':                            
                    rmenu = `${ucapanWaktu} Tod ${pushname}
...
//»»»»»»»»»»»»»»»»»»»»»»|| maker menu ||«««««««««««««««««««««««//
                    ...
                    case 'mskermenu':                            
                    mmenu = `${ucapanWaktu} Tod ${pushname}
...
//»»»»»»»»»»»»»»»»»»»»»»|| ownermenu ||«««««««««««««««««««««««//	                               
                    ...
                    case 'ownermenu':                            
                    omenu = `${ucapanWaktu} Tod ${pushname}

 ❑Owner Menu
	◈ ${prefix}owner
	◈ ${prefix}sewabot
	◈ ${prefix}bc
	◈ ${prefix}Refymodex ( BETA )
	◈ ${prefix}nsfw 0/1
	◈ ${prefix}antilink
	◈ ${prefix}antivirtex
...
//»»»»»»»»»»»»»»»»»»»»»»|| anime menu ||«««««««««««««««««««««««//
                    ...
                    case 'animemenu':                            
                    amenu = `${ucapanWaktu} Tod ${pushname}
...
${wmaing}`
	await fdhl.sendMessage(from, amenu, text, {quoted: ftrol})
	               break
                    ...
                    case 'allmenu':
                    var punya_wa = "0@s.whatsapp.net"
                    var ini_text = "𝙍𝙀𝙁𝙔 𝙈𝙀𝙉𝙐\n‣ Info Refy : status online\n‣ Rating : ★★★★☆"
...
menu2 = `Hi Tod @${pushname}👋${ucapanWaktu}
┌━━━━━━━━━━━━━━━━━━━━┈
│ ≡ *WAKTU INDONESIA*
*│⏰Wib :* ${jam}
*│⏰Wita :* ${wita}
...
case 'menu2':
 ...
 footerText: 'PEBZKUN',
 description: `Hai Tod @${sender.split('@')[0]}, Silahkan pilih menu disini`,
 sections: [
                     {
...
case 'kisahnabi':
      if (args.length < 1) return reply(`Apa Yang Mau Di Cari Tod ${pushname}\nContoh ${prefix}${command} muhammad`)
      teks = q
...
case 'github':
      if (args.length == 0) return reply(`Teks nya mana Tod ${pushname}\nExample: ${prefix + command} RezzGanz\nPastikan Nama Github Benar`)
      ini_txt = args.join(" ")
      reply(mess.wait)
...
case 'batch':
      if (args.length == 0) return reply(`Teks nya mana Tod ${pushname}\nExample: ${prefix + command} toniTodu kawai`)
      ini_txt = args.join(" ")
...
//━━━━━━━━━━━━━━━[ FITUR OWNER ]━━━━━━━━━━━━━━━━━//
     ...
     case 'bc':
         ...
         for (let _ of anu) {
         fdhl.sendMessage(_.jid, bc, image, { caption: `[ Refy mau\nIzin Broadcast ]\n\n${body.slice(4)}` })
         }
